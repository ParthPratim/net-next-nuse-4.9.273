...
struct SimTask *lib_task_create(void *private, unsigned long pid)
{
	...
	task->kernel_task.pids[__PIDTYPE_TGID].pid = kpid;
...
{
	unsigned long long ret =  (unsigned long long)(jiffies - INITIAL_JIFFIES)
	       * (NSEC_PER_SEC / HZ);
	printk("sched_clock : %llu\n",ret);
	printk("sched_clock : %d\n",ret);
	return ret;
}
